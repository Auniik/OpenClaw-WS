#!/usr/bin/env python3
"""Build a bundle for monthly DSE review.
...
def main() -> None:
    ...
    start_date = (now - timedelta(days=max(20, int(args.days)))).strftime("%Y-%m-%d")
    ...
    add_cmd("monthly-historical", ["dse-ai", "historical", "--start", start_date, "--end", end_date, "--toon"], timeout_s=180)
    ...
    add_cmd("news", ["dse-ai", "news", "--toon"], timeout_s=60)
    ...
    rc, out = _run(["dse-ai", "broker", "shanta", "portfolio-trend", "365"], timeout_s=180)
    parts.append(section("portfolio-trend-365d", out if out.strip() else f"(empty; rc={rc})"))
