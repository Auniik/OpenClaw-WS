#!/usr/bin/env python3
"""Enrich a pre-market seed bundle using a planner JSON.

This is intentionally symmetric with market-close/enrich.py so both phases have
...
# This file is a copy of scripts/market-close/enrich.py with only docstring tweaks.
...
def main() -> None:
    ...
    def _entry(mode: str, ycp: float | None, day_lo: float | None, day_hi: float | None) -> float | None:
        # Minimal heuristic: prefer ycp breakout/mean-reversion anchors; keep conservative.
        if ycp is None:
            return None
        if mode == "SHORT":
            # Short-term: near ycp or slightly above.
            return ycp
        # Swing: also anchor to ycp unless day range is informative.
        if day_lo is not None and day_hi is not None and day_hi > day_lo:
            mid = (day_lo + day_hi) / 2.0
            # If ycp is far from mid, pick closer anchor.
            return ycp if abs(ycp - mid) < 0.5 * (day_hi - day_lo) else mid
        return ycp

    def _mode(wk52_lo: float | None, wk52_hi: float | None, ycp: float | None) -> str:
        # If near 52w highs -> swing candidate, else short.
    ...
    for sym in symbols:
        ...
        entry = _entry(mode, ycp, day_lo, day_hi)

        # Stop: basic — below day low / below ycp for short; swing slightly wider.
        stop = None
        if entry is not None:
            if mode == "SHORT":
                stop = day_lo if day_lo is not None else (entry * 0.97)
            else:
                stop = wk52_lo if wk52_lo is not None else (entry * 0.94)

        r, t1, t2 = _risk_targets(entry, stop)

        # Historical (for 20D high/low + toon)
        hist = sh_json([
            "dse-ai",
            "historical",
            "--start",
