#!/usr/bin/env python3
"""Build a minimal *seed* bundle for pre-market.

Goal: before market opens, give the planner enough context (from the latest
available DSE data) to decide what to fetch (company + history) and produce a
non-hallucinated morning plan.
...
def main() -> None:
    ...
    # At pre-market, these endpoints may still represent the previous trading day.
    market_stats = sh_json(["dse-ai", "market-stats", "--json"])
    asof_date = str(market_stats.get("date") or "").strip() or run_date
    ...
    # Circuit semantics:
    # - dse-ai circuit --all  => the *universe* of instruments with circuit breaker limits
    # - dse-ai circuit --upper/--lower => instruments currently hitting upper/lower circuit
    # For trading decisions, we hard-exclude only the current hits (upper/lower).
    ...
    # Holdings (optional but extremely useful). Retry; broker endpoint can be flaky.
    # Broker portfolio can be slow/flaky; use a longer timeout so we don't silently drop holdings.
    ...
    # compliance reduce-junk (same as market-close seed)
    ...
    # Circuit semantics (important):
    # - circuit_limits_codes: instruments that have circuit breaker limits (broad universe; NOT a "do not trade" list)
    # - circuit_hit_codes: instruments currently hitting upper/lower circuit (high volatility; avoid chasing)
